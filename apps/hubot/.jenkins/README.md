...
# Build
```
( cd "$(git rev-parse --show-toplevel)/apps/hubot/.jenkins/.pipeline" && npm run build -- --pr=0 --dev-mode=true )
```
Where:
...
# Deploy to DEV
```
( cd "$(git rev-parse --show-toplevel)/apps/hubot/.jenkins/.pipeline" && npm run deploy -- --pr=0 --env=dev )
```

# Deploy to PROD
```
( cd "$(git rev-parse --show-toplevel)/apps/hubot/.jenkins/.pipeline" && npm run deploy -- --pr=0 --env=prod )
```

# Clean
The clean script can run against each persistent environment, starting from `build`.
```
( cd "$(git rev-parse --show-toplevel)/apps/hubot/.jenkins/.pipeline" && npm run clean -- --pr=0 --env=build )
( cd "$(git rev-parse --show-toplevel)/apps/hubot/.jenkins/.pipeline" && npm run clean -- --pr=0 --env=dev )
```
