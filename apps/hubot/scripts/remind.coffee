...
class Reminders
  ...
  queue: ->
    ...
    trigger = =>
      ...
      @robot.reply(reminder.msg_envelope, 'reminder to ' + reminder.action)
      @queue()
...
module.exports = (robot) ->
  ...
  robot.respond(/(remind )(.*) (in|on) (.+?) to (.*)/i, (msg) ->
    who = msg.match[2]
    type = msg.match[3]
    time = msg.match[4]
    action = msg.match[5]
    ...
    msg.send "I'll remind #{msg.envelope.user.name} to #{action} #{reminder.formatDue()}"
  )
