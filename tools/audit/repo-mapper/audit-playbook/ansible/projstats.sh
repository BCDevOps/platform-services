...
# for NS in $(oc get -o=custom-columns=NAME:.metadata.name --no-headers projects); do
  ...
  CPU_REQUEST_PC=$(awk -v A=$CPU_USAGE -v B=$CPU_REQUEST 'BEGIN { print  (A / B * 100) }')
  CPU_LIMITS_PC=$(awk -v A=$CPU_USAGE -v B=$CPU_LIMITS 'BEGIN { print  (A / B * 100) }')
  # Echo data
  echo "\"${NS}\";\"${DISPLAY_NAME}\";\"${PRODUCT_LEAD}\";\"${PRODUCT_OWNER}\";\"${NS_ENVIRONMENT}\";\"${PRODUCT}\";\"${PROJECT_TYPE}\";\"${TEAM}\";\"${CPU_USAGE}\";\"${CPU_REQUEST}\";\"${CPU_LIMITS}\";\"${CPU_REQUEST_PC}\";\"${CPU_LIMITS_PC}\"" >> $OUTPUT_PATH/project_stats.csv
done
