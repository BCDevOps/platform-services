...
for NS in $(oc get -o=custom-columns=NAME:.metadata.name --no-headers projects); do
  ...
  # Echo data
  echo "\"${NS}\";\"${DISPLAY_NAME}\";\"${PRODUCT_LEAD}\";\"${PRODUCT_OWNER}\";\"${NS_ENVIRONMENT}\";\"${PRODUCT}\";\"${PROJECT_TYPE}\";\"${TEAM}\";\"${CPU_USAGE}\";\"${CPU_REQUEST}\";\"${CPU_LIMITS}\"" >> $OUTPUT_PATH/project_stats.csv
done
